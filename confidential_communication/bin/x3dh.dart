...
Future<void> install() async {
  ...
  final signedPreKeyStoreAlice = InMemorySignedPreKeyStore();
  ...
  await signedPreKeyStoreAlice.storeSignedPreKey(signedPreKey.id, signedPreKey);

  const bobAddress = SignalProtocolAddress('bob', 1);
  final sessionBuilder = SessionBuilder(
      sessionStore, preKeyStore, signedPreKeyStoreAlice, AliceidentityStore, bobAddress);
  ...
  InitialKeyExchange(name: "test", registrationId: remoteRegId, preKeyId: remotePreKeys[0].id, preKeyPublic: remotePreKeys[0].getKeyPair().publicKey.serialize(), );
